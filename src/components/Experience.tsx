...
const Experience = () => {
  ...
  return (
    <div
      className="py-20 px-3 w-full bg-gray-50 dark:bg-transparent transition-colors duration-300"
      dir={locale === "AR" ? "rtl" : "ltr"}
    >
      <div className="text-center mb-16 relative">
        {/* Glow Behind Title */}
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="w-48 h-48 bg-white/70 dark:bg-blue-500/20 blur-3xl rounded-full animate-pulse transition-all"></div>
        </div>

        <p className="text-sm tracking-widest font-semibold uppercase relative z-10 text-blue-700 dark:text-blue-400">
          {locale === "AR" ? "خبراتنا" : "Our Expertise"}
        </p>
      ...
      <div className="w-full mt-12 grid lg:grid-cols-4 grid-cols-1 gap-10 px-6">
        {workExperience.map((card) => (
          <Button
            ...
            className="flex-1 text-black dark:text-white shadow-xl shadow-blue-900/20 hover:scale-[1.02] transition-all duration-300"
          >
            <div className="flex lg:flex-row flex-col lg:items-center p-4 md:p-6 lg:p-10 gap-4">
              ...
              <div className="lg:ms-5">
                {/* Added Title Tag for SEO */}
                <h2 className="text-start text-xl md:text-2xl font-bold text-white">
                  {locale === "AR" && card.title.AR
                    ? card.title.AR
                    : card.title.EN}
                </h2>

                {/* Optional small subtitle */}
                <h3 className="text-start text-blue-100 mt-1 font-medium">
                  {locale === "AR" && card.desc.AR
                    ? card.desc.AR
                    : card.desc.EN}
                </h3>
              </div>
